...
async def search(sequence: Sequence):
    ...
    job_response = await submit_sequence_search_job(sequence.sequence)
...
async def result(
    ...
    if actual_job_id:
        ...
        job_status_response = await get_job_status(actual_job_id)
        if job_status_response and job_status_response.status_code == HTTP_200_OK:
            # sends wait response if job is still running
            if job_status_response.content.decode() == "RUNNING":
                return JSONResponse(
                    status_code=HTTP_202_ACCEPTED,
                    content={
            ...
            elif job_status_response.content.decode() == "FINISHED":
                return await prepare_finished_job(actual_job_id, page_num)
            elif job_status_response.content.decode() == "NOT_FOUND":
                # clear the cache if the job is not found
                await delete_from_cache("sequence", job_id)
                await delete_from_cache("job_results", actual_job_id)
